...
export default function FixedExpenseEditPage() {
    ...
    const fetchData = async () => {
      ...
      try {
        const [fixedExpenseResponse, categoriesResponse] = await Promise.all([
          authenticatedFetch(`/fixed-expenses/${id}`),
          authenticatedFetch("/categories"),
        ]);

        if (!fixedExpenseResponse || !categoriesResponse) {
          return;
        }
        ...
        if (!fixedExpenseResponse.ok || !categoriesResponse.ok) {
          throw new Error("月次固定費の取得に失敗しました。");
        }
        ...
        if (!cancelled) {
          setRequestError(
            "月次固定費の取得に失敗しました。時間をおいて再度お試しください。",
          );
        }
      } finally {
        if (!cancelled) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("月次固定費の更新に失敗しました。");
      }

      setSuccessMessage("月次固定費を修正しました。");
      setTimeout(() => {
        router.push("/settings/fixed-expenses/list");
      }, 1000);
    } catch {
      setRequestError(
        "月次固定費の更新に失敗しました。時間をおいて再度お試しください。",
      );
    }
  };
      ...
      <ClientLayout>
        <main className="min-h-screen bg-gray-100 p-6">
          <div className="mx-auto max-w-md rounded bg-white p-6 shadow">
            <h1 className="mb-4 text-2xl font-bold">月次固定費修正</h1>
