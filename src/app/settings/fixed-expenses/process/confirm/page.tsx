    ...
    const fetchPreview = async () => {
      try {
        const response = await authenticatedFetch(
          `/fixed-expenses/process-preview?target_month=${encodeURIComponent(targetMonth)}`,
        );
    ...
    <ClientLayout>
      <main className="min-h-screen bg-gray-100 p-6">
        <div className="mx-auto max-w-lg rounded bg-white p-6 shadow">
          <h1 className="mb-4 text-2xl font-bold">月次固定費出金の確認</h1>
          ...
          {result ? (
            ...
            <div>
              <p className="mb-4 rounded bg-yellow-50 p-3 text-sm text-yellow-900">
                {formatTargetMonth(preview.target_month)}分として、以下の月次固定費を
                {preview.expense_date}付で出金登録しますか？
              </p>
