...
function getInitialGroupId(requestedGroupId: string | null) {
  if (requestedGroupId === null || !/^\d+$/.test(requestedGroupId)) {
    return null;
  }
...
function ExpenseListContent() {
  ...
  const [groupId, setGroupId] = useState<number | null>(() =>
    getInitialGroupId(searchParams.get("groupId")),
  );
  ...
  useEffect(() => {
    ...
    const fetchExpenses = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error("出金一覧の取得に失敗しました。");
        }
