...
export default function DashboardPage() {
  ...
  const assetChartData = stats
    ...
    : [];
  const getCashflowHref = (month: number) =>
    `/dashboard/cashflow?year=${year}&month=${month}`;
  const getAssetBalancesHref = (month: number) =>
    `/asset-balances/list?month=${year}-${String(month).padStart(2, "0")}`;
                      ...
                      >
                        ...
                        {stats.accounts.map((account, index) => (
                          ...
                          >
                            {stats.monthly.map((item) => (
                              <Cell
                                key={`${account.id}-${item.month}`}
                                role="link"
                                tabIndex={0}
                                aria-label={`${year}年${item.month}月の月次残高一覧を表示`}
                                className="cursor-pointer outline-none focus-visible:stroke-gray-900 focus-visible:stroke-2"
                                onClick={() =>
                                  router.push(
                                    getAssetBalancesHref(item.month),
                                  )
                                }
                                onKeyDown={(event) => {
                                  if (
                        ...
                        >
                          {stats.monthly.map((item) => (
                            <Cell
                              ...
                              role="link"
                              tabIndex={0}
                              aria-label={`${year}年${item.month}月の出金・入金明細を表示`}
                              className="cursor-pointer outline-none focus-visible:stroke-gray-900 focus-visible:stroke-2"
                              onClick={() =>
                                router.push(getCashflowHref(item.month))
                              }
                              onKeyDown={(event) => {
                                if (
                                  event.key === "Enter" ||
